 ...
 * 存在。于是有一整族缺陷**在本仓永远测不出来**,只有消费方会撞上(真实接入方
 * 分三轮报了六条,全部是这个形状)。
 *
 * 这个脚本不模拟,它 `npm pack` 出真 tarball、解出来按 node_modules/@vxture/*
 * 摆好,然后在那棵树上断言。三类断言各自对应一条真实付出过代价的缺陷:
 *
 *  1. **@source / @import 目标必须在包里**。`@source` 解析为空
...
for (const p of PACKAGES) {
  ...
  // 靠猜清单会漏判,真接入方就是解了真 tarball 才发现问题的。
