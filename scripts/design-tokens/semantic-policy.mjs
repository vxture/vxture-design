 ...
 *
 * 暗色不另设一套：packages/design-system/docs/01-usage.md §2 取值约束已定"暗色层级由 surface 明度递增与描边承担，
...
export const OVERLAY_WIDTHS = [
  ...
  [
    "xl",
    96,
    "宽浮层上限（= --vx-container-sm，两梯在此交接）；再宽应分栏或改用 panel",
  ],
];
...
export const SPACING_SCALE = [
  ...
  ["6xl", 14, 16, 20],
  /*
   * 7xl —— 栏间沟(column gutter)，不是元素间距。
   *
   * 上面那一梯到 6xl 为止最大 64px(默认密度)，那是「两个相邻元素之间」的量级。
   * 多栏表单要的是另一件事:**把两栏分开到视线不会串行**。opera 产品详情页的两栏
   * 字段在 64px 下，第一栏的输入框看起来仍然贴着第二栏的标签(owner 2026-09-11
   * 实页走查)。
   *
   * 默认密度给到 128px(= 第三列 32 × 4px——spacing 族的默认档取最宽那一列,
   * 见 generate-semantic-scales 的 DENSITY_MODES 注释;中间那列对本族闲置)。跨度比前面几档大是刻意的——它不是这一梯的下一格，是另一个
   * 用途的第一格;放在同一个族里只是因为它仍然由密度轴驱动。
   */
  ["7xl", 24, 28, 32],
  /* row */
  ["row-sm", 8, 10, 12],
