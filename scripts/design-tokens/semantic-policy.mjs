 ...
 * ── 间距刻度（密度三档）──
 *
 * 三列是 `--vx-spacing` 的**倍数**，依次对应 compact / default / comfortable，
 * 与 v4 的 `p-4 → calc(var(--spacing) * 4)` 同构。**三个族都按列号取值**——
 * 第几列就是第几档，没有「某族的默认档取哪一列」这种错位。
 *
 * ⚠ 三档之间是**档位平移**而非等比缩放：
 ...
 *
 * ⚠ 除 `none` 外，**每一行三档必须严格递增**。#59 就是这条没守住的后果：此前
 *   默认档的 inset / row 取的是最宽那一列（中间那列闲置），宽松档在这两族上与
 *   默认逐字相同，用户切到「宽松」只看得到控件变高。
 *
 * ⚠ `2xs` 宽松档是 1.5（6px），没有往上挪一格到 8px：那会让发丝级的图文间隙
 *   直接翻倍。
 *
 * 断言（generate-semantic-scales）：同一行三档严格递增（`none` 除外）、同一档
 * 沿族内递增。
 */
export const SPACING_SCALE = [
   ...
   * 实页走查)。
   *
   * 默认密度给到 128px(32 × 4px)。跨度比前面几档大是刻意的——它不是这一梯的
   * 下一格，是另一个用途的第一格;放在同一个族里只是因为它仍然由密度轴驱动。
   */
  ["7xl", 24, 32, 40],
  /* row */
  ["row-sm", 8, 12, 14],
