...
密度轴调三样：**留白、行高、控件高度**，三档（紧凑 / 默认 / 宽松）在每一样上都各不相同。

**幅度不同**：留白与行高是密度的主体，紧凑约减半、宽松放大一格；控件高度只动 ±1 档（±4px）。控件高度关系到点击目标和文字可读性，不能跟着留白一起大起大落。

**任何一档都不能与相邻档取值相同**。否则用户切到那一档后看不出任何变化，只会以为设置没生效。
...
- **图标顶端对齐标题首行，不加偏移**：level 2–4 的图标与标题行高同值（32/28、24/24、20/20，拉丁行高）。
