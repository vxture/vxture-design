...
| 2 租户用户工作台 | 全宽 `surface="background"`，高度用缺省 `md`（48px）      | `ShellLauncher` + `ShellHeaderMark` + `ShellHeaderTitle`（平台名 vxture.ai）+ `ShellHeaderDivider` + `ShellScopeButton`（当前租户，不带图标）                                                                                               | `ShellSearchBox` + `ShellAgentButton` + `ShellToolbox`（帮助 / 通知 / 设置）+ `ShellUserMenu`                                                                                   |
