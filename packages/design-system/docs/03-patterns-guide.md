...
| AlertDialog | 强制二选一                                        | 无 X、Esc 与遮罩不关，用户必须表态；不可撤销的落锤给 Action 传 `destructive-strong`。**破坏性确认不要直接用它**，用 `ConfirmDestructive`（§3.1） |
...
| 参数            | 作用                                                             |
| --------------- | ---------------------------------------------------------------- |
| `verb`          | 动作名，必须是动词本身。同时进标题与确认钮——写不出「确定」       |
| `target`        | 对象。与 verb 拼成标题「{verb}{target}？」                       |
