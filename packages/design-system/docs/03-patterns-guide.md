...
| ---------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 官网           | `layout="centered"` `surface="transparent"` `height="xl"` | `ShellHeaderMark` + `ShellHeaderTitle`（站名）；产品页再加 `ShellHeaderDivider` + `ShellProductTitle`（标识 / 名称 / 类型）                                                                                                                 | 未登录：`ShellHeaderTools`（只给主题 / 语言 / 全屏）+ 注册 / 登录（`Button size="sm"`）；产品页把注册 / 登录换成「官网」；已登录：`ShellHeaderTools`（六件）+ `ShellUserMenu` |
| 2 租户用户工作台 | 全宽 `surface="background"`，高度用缺省 `md`（48px）      | `ShellLauncher` + `ShellHeaderMark` + `ShellHeaderTitle`（平台名 vxture.ai）+ `ShellHeaderDivider` + `ShellScopeButton`（当前租户，不带图标）                                                                                               | `ShellSearchBox` + `ShellAgentButton` + `ShellHeaderTools`（六件）+ `ShellUserMenu`                                                                                           |
| 3 平台管理工作台 | 同 2                                                      | 同 2，标题同为平台名，但**必须带管理员徽标**（`ShellHeaderTitle badge`，实底 `Badge`，上标位）；分隔线后是 `ShellHeaderDomain`（当前域），不放租户选择                                                                                      | 同 2                                                                                                                                                                          |
| 4 单产品视角     | 同 2                                                      | `ShellIconButton icon="sidebar"` + `ShellLauncher` + `ShellHeaderMark` + `ShellHeaderDivider` + `ShellProductTitle`（标识 / 名称 / 类型 / 上标等级，等级用 `StatusBadge size="sm"`）+ `ShellHeaderDivider` + `ShellScopeButton`（当前租户） | 同 2                                                                                                                                                                          |
...
- **工具箱常态透明**：`ShellToolbox` 平时无底，悬停或键盘焦点落进组内时才整组亮底、指针所在的工具再亮一层；不要给它补底色。
- **工具箱用 `ShellHeaderTools`，顺序与交互固定**（owner 2026-09-26）：① 主题（点击切换）② 语言（弹出面板选择）③ 全屏（直接切换）④ 帮助（新标签页，定位到**当前页面**的帮助主题）⑤ 消息（侧边抽屉）⑥ 配置（跳转后台配置）。门户只决定给哪几件：访客给前三件，登录后给全部六件。不要用 `ShellToolbox` 自己排顺序——六个门户各排一遍迟早不一样。
- **两种管理工作台靠徽标与分隔线后的内容区分视角**（标题都是平台名）：租户工作台是「租户用户管理自己」，分隔线后是当前租户（可切换）；平台管理工作台是「平台员工管理平台或全部租户」，标题同为平台名，靠管理员徽标区分，分隔线后是当前管理域。两者同屏出现在截图里要一眼分得开。
- **徽标与等级读作上标**：`ShellHeaderTitle` 的 `badge`、`ShellProductTitle` 的 `tier` 都放在顶端对齐的版位里（徽标 24px、等级 32px），比标题略高——它们是标题的注脚，不是并列的第二个名字。
