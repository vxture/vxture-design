...
| 包                       | 版本  | 职责                                                              | 依赖              |
| ------------------------ | ----- | ----------------------------------------------------------------- | ----------------- |
| `@vxture/design-tokens`  | 2.2.0 | T1 原始层（Tailwind theme 的完整镜像）+ T2 语义层                 | 无 `@vxture` 依赖 |
| `@vxture/design-ui`      | 3.1.1 | 组件实现                                                          | design-tokens     |
| `@vxture/design-system`  | 6.4.0 | 伞包 + 运行时接线（主题 / 密度 / 字号 provider、shell、品牌样式） | tokens + ui       |
...
（`@vxture/design-system` 已到 6.4.0，从 0.1.0 重开会让 `latest` 倒退）。
